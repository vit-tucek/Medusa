/*  Author Christian Henriksen 1998,
    in corporation with Kuan Ju Liu


    Code for manipulating Medusas, the generalization of spiders
    for quadratic rational mappings
...
/* ****************************************
** Misc. function declartions
**************************************** */


CComplex reciprok(CComplex z)
  // Return 1 / z when z is not close to 0
  // otherwise returns (the constant) infinity
...
/* ****************************************
** Definitions for the class Medusa_comp
**************************************** */

CComplex Medusa_comp::position(void) {
  return z;
}
...
/* ****************************************
** // Definition for the class Medusa
**************************************** */
...
void Medusa::findorbit(long int p, long int q, DynArray<long int>& table, 
...
void Medusa::find_kneading_data(long int q, int period, int preperiod,
		     DynArray<long int>& table, 
...
void Medusa::find_succ(long int p, long int q, DynArray<long int>& table,
		int& ok, long int& pp, int& legno)
{
  pp = q;
  int j;
...
void Medusa::init_body(DynArray<long int>& table1, DynArray<long int>& table2, long int q)
{
  ...
  Medusa_comp* act_p = body_p;
  int i_ok, i_legno, o_ok, o_legno;
  long int p1, p2;
  long int p = -1;
  ...
  while (i_ok || o_ok)
	...
	// two legs.
	{
	  ...
	  act_p->next_component = 
	    (Medusa_comp*)(new Joint(exp(CComplex(0,2.0*M_PI*p/q)), 
			      i_legno, o_legno));
...
void Medusa::invf(CComplex refpos, CComplex u, CComplex& z1, CComplex& z2)
{
...
void Medusa::pullback_leg(int from_leg, int to_leg, 
...
void Medusa::pullback(DynArray<CComplex>& dist_pts)
{
  dist_pts.resetmaxassigned();

  Medusa_comp* body_A_p = new Segment (1);
  Medusa_comp* body_B_p = new Segment (-1);
  
  Medusa_comp* A_p = body_A_p;
  Medusa_comp* B_p = body_B_p;

  Medusa_comp* old_p = body_p->next_component;
  ...
  Medusa_comp* ptr = (old_p = body_p->next_component);
  while(ptr!=body_p)
    {
      old_p = old_p->next_component;
      delete ptr;
...
void Medusa::find_params_and_cvs(CComplex& omega1, CComplex& omega2)
...
{
  Medusa_comp* ptr = o_legs[0].leg_p;
...
int Medusa::rectify(DynArray<CComplex> dist_pts, 
		     CComplex& omega1, CComplex& omega2)
  // Rectifies the Medusa, adding intermidiate segments if necessary
...
void Medusa::prune_legs(int nooflegs, const Leg_list legtable, 
			CComplex& omega,
			DynArray<CComplex>& dist_pts)
{
 int j;
  ...
  for(j=0; j<nooflegs; j++)
    {
      ...
      Medusa_comp* ptr = legtable[j].leg_p;
      while(ptr->next_component != 0 && 
...
void Medusa::prune(DynArray<CComplex>& dist_pts,
		   CComplex& omega1, CComplex& omega2)
...
void Medusa::circle_connect(ostream& s, CComplex a, 
...
Medusa::Medusa(long int p1, long int p2, long int q) 
{  
  ...
  DynArray<long int> table1(0);
  findorbit(p1, q, table1, inner_period, inner_preperiod);
  DynArray<long int> table2(0);
...
ostream& operator << (ostream& s, const Medusa& c)
{
  // Outputs the medusa to s, in a format compatible with
  // Maple.
